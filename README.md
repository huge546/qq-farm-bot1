...
基于 Node.js 的 QQ 农场自动化工具，提供多账号挂机、农场与好友自动化、作物与超变图鉴、活动中心、商城、数据分析和 Web 控制面板。当前项目版本为 `20260901`，游戏协议版本为 `1.13.3.16_20260826`。
...
- 支持多租户卡密体系：普通用户通过卡密注册、登录与兑换，账号所属隔离与配额管理
...
管理员账号不通过环境变量初始化。首次运行会自动创建 `admin` / `admin`，之后的管理员凭据保存在数据目录的 `admin.json` 中。普通用户可在登录页通过卡密注册；管理员可在“卡密管理”页生成账号额度卡密与时长卡密。
...
游戏账号绑定注册用户；普通用户只能管理自己的账号，并受账号配额上限约束（可通过兑换账号额度卡密提升），时长卡到期后恢复基础配额。下线提醒同样是实例级配置，对当前实例内的所有游戏账号生效。
